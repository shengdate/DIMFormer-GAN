"""
DIMFormer-GAN / Mask2Former-GAN core network components.

Main components:
1. CNN backbone with residual and deformable convolution blocks.
2. FPN-like pixel decoder for multi-scale feature fusion.
3. Masked-attention Transformer decoder for instance prediction.
4. Semantic segmentation branch for foreground guidance.
5. Semantic-constrained instance aggregation module.
6. Conditional PatchGAN-style discriminator.
7. Hungarian matching and segmentation losses.
8. Adversarial and semantic-instance consistency losses.
"""
...
class Mask2Encoder(nn.Module):
    def __init__(self, num_queries=10, num_classes=1, hidden_dim=128, mask_dim=128, nheads=8, num_layers=8,):
        super().__init__()
...
class Mask2FormerGANGenerator(nn.Module):
    def __init__( self, in_channels=3, num_queries=10, hidden_dim=128, mask_dim=128, num_classes=1, num_decoder_layers=8,):
        super().__init__()
...
# Basic convolution block for the discriminator
...
class semationmask(nn.Module):
    ...
    def forward(self, outputs):
        ...
        # Class 1 represents a dispersion-energy instance
        ...
        # Apply the semantic foreground probability as a gate to instance masks
        constrained_masks = mask_prob * sem_for_gate
        # Soft union of the original instance masks
        raw_union = self.soft_union(masks=mask_prob,scores=fg_scores,)
        # Soft union of the semantic-constrained instance masks
...
# Pairwise mask Dice cost used for instance matching
...
# Pairwise mask BCE cost used for instance matching
...
class GeneratorTotalLoss(nn.Module):
    ...
    def forward(self, outputs, targets, sem_gt, image, D=None, use_adv=True):
        ...
        # 2. Instance segmentation loss with Hungarian matching
        ins_losses = self.instance_criterion( outputs, targets)
        # 3. Generate semantic-constrained instance predictions
...
# Build training targets from an instance mask
